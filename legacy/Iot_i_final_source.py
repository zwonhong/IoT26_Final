...
try:
    ...
    USING_INFERENCE_SDK = False

    class InferenceHTTPClient:
        def __init__(self, api_url, api_key):
            self.api_url = api_url.rstrip("/")
            self.api_key = api_key
            print("⚠️ inference_sdk 없음: requests fallback 클라이언트 사용")
        ...
        def _encode_image(self, image):
            ...
            if not success:
                raise ValueError("Roboflow 전송용 이미지 인코딩 실패")
...
if not ROBOFLOW_API_KEY:
    print("❌ ROBOFLOW_API_KEY가 설정되지 않았습니다")
    print("Roboflow 모델을 쓰려면 먼저 아래처럼 API 키를 설정하세요:")
    print('export ROBOFLOW_API_KEY="여기에_본인_Roboflow_API_Key"')
    sys.exit(1)
...
print("🧠 Roboflow Hosted API 모델 사용")
print(f"📡 model_id={ROBOFLOW_MODEL_ID}")
print(f"📦 inference_sdk 사용: {USING_INFERENCE_SDK}")
print("🎨 RGB/BGR 색상 보정 테스트 모드")
...
def predict_with_roboflow(frame_bgr):
    try:
        result = CLIENT.infer(
            frame_bgr,
        ...
        return [
            ...
            if float(prediction.get("confidence", 0.0)) >= CONF_THRESHOLD
        ]

    except requests.RequestException as e:
        print("⚠️ Roboflow 요청 실패:", e)
    except (OSError, ValueError, TypeError) as e:
        print("⚠️ Roboflow 처리 실패:", e)
    except Exception as e:
        print("⚠️ Roboflow SDK 요청 실패:", e)
...
def normalize_display_name(class_name):
    ...
    if comparable_name in ("plastic bag", "plastic cup", "plastic bottle"):
        return "plastic"
...
def get_lcd_display_name(display_name):
    ...
    if "aluminum" in comparable_name or "can" in comparable_name:
        return "Can"
...
def get_servo_category(display_name):
    ...
    if "can" in comparable_name or "metal" in comparable_name:
        return "metal"
    ...
    while True:
        ...
        if distance_cm < DETECT_DISTANCE_CM:
            ...
            while time.time() - scan_start < SCAN_SECONDS:
                ...
                predictions = predict_with_roboflow(frame_fixed)
                ...
                for prediction in predictions:
                    ...
                    print(
                        f"DEBUG Roboflow detect: id={cls_id}, "
            ...
            if detected_name:
                ...
                print("⚠️ 분류 실패: Roboflow 감지 결과 없음")
